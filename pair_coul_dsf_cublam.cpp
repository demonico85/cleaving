...
#include <cstdlib>
#include <cstring>
#include "pair_coul_dsf_cublam.h"
#include "atom.h"
#include "comm.h"
#include "force.h"
#include "neighbor.h"
...
PairCoulCubLDSF::PairCoulCubLDSF(LAMMPS *lmp) : Pair(lmp) {
...
PairCoulCubLDSF::~PairCoulCubLDSF()
{
  if (copymode) return;
...
void PairCoulCubLDSF::compute(int eflag, int vflag)
{
  int i,j,ii,jj,inum,jnum,itype,jtype,m;
  double qtmp,xtmp,ytmp,ztmp,delx,dely,delz,ecoul,fpair;
  double r,rsq,r2inv,forcecoul,factor_coul,flam;
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cut_coulsq) {
        ...
        fpair = flam * flam * flam * forcecoul * r2inv;
	   ...
	   if(scaling){
             pvector[m] += 3.0 * flam * flam * ecoul;
             ecoul *= flam * flam * flam ;
            }
...
void PairCoulCubLDSF::global_boundary(){
...
int PairCoulCubLDSF::find_scaling(int imvec, int jmvec, int i, int j,double *xj){
...
void PairCoulCubLDSF::settings(int narg, char **arg)
{
...
void PairCoulCubLDSF::coeff(int narg, char **arg)
{
...
void PairCoulCubLDSF::init_style()
{
  if (!atom->q_flag)
...
double PairCoulCubLDSF::init_one(int i, int j)
{
  lam[j][i] = lam[i][j];
...
void PairCoulCubLDSF::write_restart(FILE *fp)
{
...
void PairCoulCubLDSF::read_restart(FILE *fp)
{
...
void PairCoulCubLDSF::write_restart_settings(FILE *fp)
{
...
void PairCoulCubLDSF::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
double PairCoulCubLDSF::single(int i, int j, int itype, int jtype, double rsq,
...
void *PairCoulCubLDSF::extract(const char *str, int &dim)
{
