...
#include "pair_lj_cut_tip4p_long_cleav_opt.h"
...
PairLJCutTIP4PLongCleavOpt::PairLJCutTIP4PLongCleavOpt(LAMMPS *lmp) :
...
PairLJCutTIP4PLongCleavOpt::~PairLJCutTIP4PLongCleavOpt()
{
...
void PairLJCutTIP4PLongCleavOpt::allocate()
{
  allocated = 1;
  int n = atom->ntypes;
...
void PairLJCutTIP4PLongCleavOpt::compute(int eflag, int vflag)
{
...
/* ---------------------------------------------------------------------- */
void PairLJCutTIP4PLongCleavOpt::computeLJ(int eflag, int vflag)
{
...
/* ---------------------------------------------------------------------- */
void PairLJCutTIP4PLongCleavOpt::computeC(int eflag, int vflag)
{
  int i,j,ii,jj,inum,jnum,itype,jtype,itable,key;
  int n,vlist[6];
  int iH1,iH2,jH1,jH2;
...
void PairLJCutTIP4PLongCleavOpt::global_boundary(){
...
int PairLJCutTIP4PLongCleavOpt::find_scaling(int imvec, int jmvec, int i, int j,double *xj){
...
void PairLJCutTIP4PLongCleavOpt::settings(int narg, char **arg)
{
  if (narg < 12 || narg > 15) error->all(FLERR,"Illegal pair_style command");
...
if (ccom && (strcmp(idchunk,ccom->idchunk) != 0))
 ...
 posbordertypes = utils::numeric(FLERR,arg[10],false,lmp);
  ...
  if (narg > 12)
  	cut_coul = utils::numeric(FLERR,arg[12],false,lmp);
  ...
  if (narg > 13){ 
    switchcoul = utils::inumeric(FLERR,arg[13],false,lmp);  
    switchlj = utils::inumeric(FLERR,arg[14],false,lmp);
	}
...
void PairLJCutTIP4PLongCleavOpt::init_style()
{
...
void PairLJCutTIP4PLongCleavOpt::coeff(int narg, char **arg)
{
  if (narg < 4 || narg > 7)
...
double PairLJCutTIP4PLongCleavOpt::init_one(int i, int j)
{
...
void PairLJCutTIP4PLongCleavOpt::write_restart_settings(FILE *fp)
{
...
void PairLJCutTIP4PLongCleavOpt::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
void PairLJCutTIP4PLongCleavOpt::compute_newsite(double *xO, double *xH1,
...
void *PairLJCutTIP4PLongCleavOpt::extract(const char *str, int &dim)
{
  dim = 0;
...
double PairLJCutTIP4PLongCleavOpt::memory_usage()
{
