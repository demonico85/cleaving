...
FixWall::FixWall(LAMMPS *lmp, int narg, char **arg) : Fix(lmp, narg, arg), nwall(0)
{
  ...
  while (iarg < narg) {
    ...
    if (utils::strmatch(style, "^wall/lj126/cleav")) wantargs = 7;    
    ...
    if ((strcmp(arg[iarg], "xlo") == 0) || (strcmp(arg[iarg], "xhi") == 0) ||
      ...
      if (utils::strmatch(style, "^wall/lepton")) {
        ...
        cutoff[nwall] = utils::numeric(FLERR, arg[iarg + 4], false, lmp);
        if (utils::strmatch(style, "^wall/BG/cleav")){
         cutoffout[nwall] = utils::numeric(FLERR, arg[iarg + 5], false, lmp);
         lambda[nwall] = utils::numeric(FLERR, arg[iarg + 6], false, lmp); }
      }
      nwall++;
      iarg += wantargs;
