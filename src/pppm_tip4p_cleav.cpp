...
PPPMTIP4PCleav::PPPMTIP4PCleav(LAMMPS *lmp) : PPPM(lmp)
{
  triclinic_support = 1;
  tip4pflag = 1;
}
...
void PPPMTIP4PCleav::settings(int narg, char **arg)
{
  if (narg < 2) error->all(FLERR,"Illegal kspace_style {} command", force->kspace_style);
...
void PPPMTIP4PCleav::particle_map()
{
...
void PPPMTIP4PCleav::fieldforce_ik()
{
  ...
  for (i = 0; i < nlocal; i++) {
    ...
    const double qfactor = lambda * qqrd2e * scale * q[i];
...
void PPPMTIP4PCleav::fieldforce_ad()
{
  int i,l,m,n,nx,ny,nz,mx,my,mz;
  FFT_SCALAR dx,dy,dz;
  FFT_SCALAR ekx,eky,ekz;
  ...
  for (i = 0; i < nlocal; i++) {
    ...
    const double qfactor = lambda * qqrd2e * scale;
...
void PPPMTIP4PCleav::fieldforce_peratom()
{
  int i,l,m,n,nx,ny,nz,mx,my,mz;
  FFT_SCALAR dx,dy,dz,x0,y0,z0;
  double *xi;
...
void PPPMTIP4PCleav::find_M(int i, int &iH1, int &iH2, double *xM)
{
  double **x = atom->x;
