...
#include "pair_lj_cut_tip4p_cut_cleav.h"
...
PairLJCutTIP4PCutCleav::PairLJCutTIP4PCutCleav(LAMMPS *lmp) : Pair(lmp)
{
  single_enable = 0;
  writedata = 1;
...
//fp = fopen("cleav.log","w");

}

/* ---------------------------------------------------------------------- */

PairLJCutTIP4PCutCleav::~PairLJCutTIP4PCutCleav()
{
  if (allocated) {
...
void PairLJCutTIP4PCutCleav::compute(int eflag, int vflag)
{
  int i,j,ii,jj,inum,jnum,itype,jtype;
  double qtmp,xtmp,ytmp,ztmp,delx,dely,delz,evdwl,ecoul,flam,fDlam;
  double rsq,r2inv,r6inv,forcecoul,forcelj,factor_lj,factor_coul,fpair;
...
//  tagint *molecule = atom->molecule;
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cut_ljsq[itype][jtype] && lj1[itype][jtype] > 0.0) {
        r2inv = 1.0/rsq;
        ...
        fpair = r6inv * (lj1[itype][jtype]*r6inv - lj2[itype][jtype]);
        forcelj =  flam * factor_lj * fpair * r2inv;
...
// It can be avoided if we enforce coul_cut_off <= lj_cut_off
          ...
          cforce = flam * factor_coul * forcecoul * r2inv;
...
void PairLJCutTIP4PCutCleav::global_boundary(){
...
int PairLJCutTIP4PCutCleav::find_scaling(int imvec, int jmvec, int i, int j,double *xj){
...
void PairLJCutTIP4PCutCleav::allocate()
{
  allocated = 1;
  int n = atom->ntypes;
...
void PairLJCutTIP4PCutCleav::coeff(int narg, char **arg)
{
  if (narg < 4 || narg > 7)
...
void PairLJCutTIP4PCutCleav::init_style()
{
...
double PairLJCutTIP4PCutCleav::init_one(int i, int j)
{
  if (setflag[i][j] == 0) {
...
void PairLJCutTIP4PCutCleav::write_restart(FILE *fp)
{
...
void PairLJCutTIP4PCutCleav::read_restart(FILE *fp)
{
...
void PairLJCutTIP4PCutCleav::write_restart_settings(FILE *fp)
{
...
void PairLJCutTIP4PCutCleav::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
void PairLJCutTIP4PCutCleav::write_data(FILE *fp)
{
...
void PairLJCutTIP4PCutCleav::write_data_all(FILE *fp)
{
  for (int i = 1; i <= atom->ntypes; i++)
...
void PairLJCutTIP4PCutCleav::compute_newsite(double *xO,  double *xH1,
...
void *PairLJCutTIP4PCutCleav::extract(const char *str, int &dim)
{
  dim = 0;
...
double PairLJCutTIP4PCutCleav::memory_usage()
{
