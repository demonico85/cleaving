...
void PairLJCutTIP4PDsf::compute(int eflag, int vflag)
{
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    if (eflag) {
      double e_self = -(e_shift / 2.0 + alpha_dsf / MY_PIS) * qtmp * qtmp * qqrd2e;
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cut_coulsqplus) {
        ...
        if (rsq < cut_coulsq) {
          ...
          erfcd = exp(-alpha_dsf * alpha_dsf * rsq);
          t = 1.0 / (1.0 + EWALD_P * alpha_dsf * r);
          erfcc = t * (A1 + t * (A2 + t * (A3 + t * (A4 + t * A5)))) * erfcd;

          forcecoul = prefactor * (erfcc / r + 2.0 * alpha_dsf / MY_PIS * erfcd + r * f_shift) * r;
...
void PairLJCutTIP4PDsf::settings(int narg, char **arg)
{
  ...
  alpha_dsf = utils::numeric(FLERR, arg[5], false, lmp);
...
void PairLJCutTIP4PDsf::write_data(FILE *fp)
{
...
void PairLJCutTIP4PDsf::write_data_all(FILE *fp)
{
  for (int i = 1; i <= atom->ntypes; i++)
...
void *PairLJCutTIP4PDsf::extract(const char *str, int &dim)
{
  dim = 0;
