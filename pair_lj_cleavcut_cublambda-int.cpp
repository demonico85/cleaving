...
#include <stdlib.h>
#include <string.h>
#include "pair_lj_cleavcut_cublambda-int.h"
#include "atom.h"
#include "comm.h"
#include "force.h"
#include "neighbor.h"
...
PairLJCleavCutCubLInt::PairLJCleavCutCubLInt(LAMMPS *lmp) : Pair(lmp) , idflag(NULL)
{
...
PairLJCleavCutCubLInt::~PairLJCleavCutCubLInt()
{
...
void PairLJCleavCutCubLInt::compute(int eflag, int vflag)
{
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < radm[itype][jtype]){
          ...
          fpair = flam * flam * flam * factor_lj * forcelj;
          ...
          if (eflag) {
            ...
            evdwl *= factor_lj;
            if(scaling){
              pvector[m] += 3.0 * flam * flam * evdwl;
              evdwl *= flam*flam*flam;
              }
          }
        ...
        fpair = flam * flam * flam * factor_lj * forcelj * r2inv;
...
void PairLJCleavCutCubLInt::global_boundary(){
...
int PairLJCleavCutCubLInt::find_scaling(int imvec, int jmvec, int i, int j,double *xj){
...
void PairLJCleavCutCubLInt::allocate()
{
  allocated = 1;
  int n = atom->ntypes;
...
void PairLJCleavCutCubLInt::settings(int narg, char **arg)
{
...
void PairLJCleavCutCubLInt::coeff(int narg, char **arg)
{
  if (narg < 4 || narg > 6)
...
void PairLJCleavCutCubLInt::init_style()
{
...
void PairLJCleavCutCubLInt::calculate_int_coefficients()
{
...
double PairLJCleavCutCubLInt::init_one(int i, int j)
{
  if (setflag[i][j] == 0) {
  ...
  if (offset_flag) {
    ...
    offset[i][j] =  lam[i][j] * lam[i][j] * lam[i][j] * 4.0 * epsilon[i][j] * (pow(ratio,12.0) - pow(ratio,6.0));
  } else offset[i][j] = 0.0;
...
void PairLJCleavCutCubLInt::write_restart(FILE *fp)
{
...
void PairLJCleavCutCubLInt::read_restart(FILE *fp)
{
...
void PairLJCleavCutCubLInt::write_restart_settings(FILE *fp)
{
...
void PairLJCleavCutCubLInt::read_restart_settings(FILE *fp)
{
  int me = comm->me;
  if (me == 0) {
...
void PairLJCleavCutCubLInt::write_data(FILE *fp)
{
...
void PairLJCleavCutCubLInt::write_data_all(FILE *fp)
{
  for (int i = 1; i <= atom->ntypes; i++)
...
double PairLJCleavCutCubLInt::single(int i, int j, int itype, int jtype, double rsq,
                         ...
                         double &fforce)
{
  ...
  if (rsq < radm[itype][jtype]){
      rfourth=rsq*rsq;
      forcelj = lju3[itype][jtype] + lju4[itype][jtype]*rsq;
      fforce = lam[itype][jtype]*lam[itype][jtype]*lam[itype][jtype] * factor_lj * forcelj;
      philj =  lju0[itype][jtype]+lju1[itype][jtype]*rsq+lju2[itype][jtype]*rfourth;
      philj = lam[itype][jtype]*lam[itype][jtype]*lam[itype][jtype]*philj;
  }
  else {
  r2inv = 1.0/rsq;
  r6inv = r2inv*r2inv*r2inv;
  forcelj = r6inv * (lj1[itype][jtype]*r6inv - lj2[itype][jtype]);
  fforce = lam[itype][jtype]*lam[itype][jtype]*lam[itype][jtype]*factor_lj*forcelj*r2inv;
...
void *PairLJCleavCutCubLInt::extract(const char *str, int &dim)
{
  dim = 2;
